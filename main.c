...
#define VP_VALVE_OPEN_3    0x1931   // 档位3开度百分比
#define VP_VALVE_OPEN_4    0x1941   // 档位4开度百分比
#define VP_VALVE_OPEN_5    0x1951   // 档位5开度百分比
...
#define VP_DBG_OUT_VOLT_X10 0x1020   // 当前输出电压指令值，单位 0.1V：30=3.0V，100=10.0V
...
static u16 valve_output_percent_for_fan(u16 fan)
{
    ...
    */
    ...
    switch(fan)
    {
        case 1: addr = VP_VALVE_OPEN_1; break;
        case 2: addr = VP_VALVE_OPEN_2; break;
        case 3: addr = VP_VALVE_OPEN_3; break;
        case 4: addr = VP_VALVE_OPEN_4; break;
        case 5: addr = VP_VALVE_OPEN_5; break;
        default: addr = VP_VALVE_OPEN_4; break;
    }

    percent = 100;
    sys_read_vp(addr, (u8 *)&percent, 1);
...
static void write_debug_values(void)
{
    ...
    /*
       当前输出电压调试值：
       output_percent_now 是输出百分比，0~100。
       对 0~10V 来说，百分比数值刚好等于 电压x10：
       30 -> 3.0V，50 -> 5.0V，100 -> 10.0V。
       前端数据显示绑定 0x1020，小数位数设 1，即可显示 0.0~10.0V。
    */
    dbg_output_volt_x10 = output_percent_now;
    sys_write_vp(VP_DBG_OUT_VOLT_X10, (u8 *)&dbg_output_volt_x10, 1);
